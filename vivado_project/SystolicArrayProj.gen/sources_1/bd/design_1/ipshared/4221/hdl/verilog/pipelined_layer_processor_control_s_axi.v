...
// ==============================================================
`timescale 1ns/1ps
module pipelined_layer_processor_control_s_axi
#(parameter
    ...
    output wire [31:0]                   num_row_tiles,
    output wire [31:0]                   num_depth_tiles,
    output wire [31:0]                   num_col_tiles,
...
localparam
    ...
    ADDR_NUM_ROW_TILES_DATA_0   = 7'h34,
    ADDR_NUM_ROW_TILES_CTRL     = 7'h38,
    ADDR_NUM_DEPTH_TILES_DATA_0 = 7'h3c,
    ADDR_NUM_DEPTH_TILES_CTRL   = 7'h40,
    ADDR_NUM_COL_TILES_DATA_0   = 7'h44,
    ADDR_NUM_COL_TILES_CTRL     = 7'h48,
    WRIDLE                      = 2'd0,
    WRDATA                      = 2'd1,
    WRRESP                      = 2'd2,
    WRRESET                     = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [31:0]                   int_num_row_tiles = 'b0;
    reg  [31:0]                   int_num_depth_tiles = 'b0;
    reg  [31:0]                   int_num_col_tiles = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_NUM_ROW_TILES_DATA_0: begin
                    rdata <= int_num_row_tiles[31:0];
                end
                ADDR_NUM_DEPTH_TILES_DATA_0: begin
                    rdata <= int_num_depth_tiles[31:0];
                end
                ADDR_NUM_COL_TILES_DATA_0: begin
                    rdata <= int_num_col_tiles[31:0];
                end
            endcase
        end
    end
...
assign num_row_tiles     = int_num_row_tiles;
assign num_depth_tiles   = int_num_depth_tiles;
assign num_col_tiles     = int_num_col_tiles;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
// int_num_row_tiles[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_num_row_tiles[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_ROW_TILES_DATA_0)
            int_num_row_tiles[31:0] <= (WDATA[31:0] & wmask) | (int_num_row_tiles[31:0] & ~wmask);
    end
end

// int_num_depth_tiles[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_num_depth_tiles[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_DEPTH_TILES_DATA_0)
            int_num_depth_tiles[31:0] <= (WDATA[31:0] & wmask) | (int_num_depth_tiles[31:0] & ~wmask);
    end
end

// int_num_col_tiles[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_num_col_tiles[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_NUM_COL_TILES_DATA_0)
            int_num_col_tiles[31:0] <= (WDATA[31:0] & wmask) | (int_num_col_tiles[31:0] & ~wmask);
    end
end
