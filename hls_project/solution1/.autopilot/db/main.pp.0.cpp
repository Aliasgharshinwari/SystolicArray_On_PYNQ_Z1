...
# 2 "<built-in>" 2
# 1 "SystolicArray/main.cpp" 2
# 1 "SystolicArray/main.h" 1
...
# 5 "SystolicArray/main.h" 2
# 16 "SystolicArray/main.h"
...
__attribute__((sdx_kernel("pipelined_layer_processor", 0))) void pipelined_layer_processor(
    vec_t *mat_A,
    vec_t *mat_B,
    vec_out_t *mat_C,
    int num_row_tiles, int num_depth_tiles, int num_col_tiles
);
# 2 "SystolicArray/main.cpp" 2
...
void read_big_tile(
    vec_t *dram_matrix,
    input_t local_buffer[32][32],
    int tile_index
) {
#pragma HLS INLINE off

 int start_addr = tile_index * (32 * 32 / 32);

    Read_Loop: for(int i=0; i<(32 * 32 / 32); i++) {
#pragma HLS PIPELINE II=1

 vec_t packet = dram_matrix[start_addr + i];

        Unpack: for(int k=0; k<32; k++) {
#pragma HLS UNROLL
...
void compute_systolic(
    input_t big_tile_A[32][32],
    input_t big_tile_B[32][32],
    output_t big_tile_C[32][32]
) {
#pragma HLS INLINE off

 input_t h_wires[32][32 + 1];
    input_t v_wires[32 + 1][32];
#pragma HLS ARRAY_PARTITION variable=h_wires dim=0 complete
#pragma HLS ARRAY_PARTITION variable=v_wires dim=0 complete


 output_t local_regs[32][32];
#pragma HLS ARRAY_PARTITION variable=local_regs dim=0 complete


 Scan_Row: for(int tr = 0; tr < 32; tr += 32) {
        Scan_Col: for(int tc = 0; tc < 32; tc += 32) {


            Load_Cache_Row: for(int r=0; r<32; r++) {
#pragma HLS PIPELINE II=1
 Load_Cache_Col: for(int c=0; c<32; c++) {
#pragma HLS UNROLL
 local_regs[r][c] = big_tile_C[tr+r][tc+c];
            ...
            VITIS_LOOP_60_1: for(int i=0; i<32; i++) VITIS_LOOP_60_2: for(int j=0; j<32; j++)
#pragma HLS UNROLL
 h_wires[i][j] = v_wires[i][j] = 0;


            Pulse: for(int k = 0; k < 32; k++) {
#pragma HLS PIPELINE II=1

 VITIS_LOOP_68_3: for(int r=0; r<32; r++) h_wires[r][0] = big_tile_A[tr + r][k];
                VITIS_LOOP_69_4: for(int c=0; c<32; c++) v_wires[0][c] = big_tile_B[k][tc + c];

                Grid_Row: for(int r=0; r<32; r++) {
#pragma HLS UNROLL
 Grid_Col: for(int c=0; c<32; c++) {
#pragma HLS UNROLL
...
#pragma HLS BIND_OP variable=prod op=mul impl=dsp
 ...
 local_regs[r][c] += prod;
            ...
            Store_Cache_Row: for(int r=0; r<32; r++) {
#pragma HLS PIPELINE II=1
 Store_Cache_Col: for(int c=0; c<32; c++) {
#pragma HLS UNROLL
...
void write_big_tile(
    output_t local_buffer[32][32],
    vec_out_t *dram_matrix,
    int tile_index
) {
#pragma HLS INLINE off

 int start_addr = tile_index * (32 * 32 / 8);

    Write_Rows: for(int r=0; r<32; r++) {
        Write_Cols: for(int c=0; c<32; c += 8) {
#pragma HLS PIPELINE II=1

 vec_out_t packet = 0;
            Pack: for(int k=0; k<8; k++) {
#pragma HLS UNROLL
 packet.range((k*32)+31, k*32) = local_buffer[r][c + k];
            }

            int offset = (r * (32/8)) + (c/8);
...
#line 16 "/home/ali/Desktop/SystolicArray/solution1/csynth.tcl"
#pragma HLSDIRECTIVE TOP name=pipelined_layer_processor
# 135 "SystolicArray/main.cpp"

#line 6 "/home/ali/Desktop/SystolicArray/solution1/directives.tcl"
#pragma HLSDIRECTIVE TOP name=pipelined_layer_processor
# 135 "SystolicArray/main.cpp"
...
#pragma HLS INTERFACE s_axilite port=mat_C bundle=control
 ...
 input_t tile_A[32][32];
    input_t tile_B[32][32];
    output_t tile_C[32][32];
...
#pragma HLS BIND_STORAGE variable=tile_C type=ram_2p impl=bram
 ...
 Row_Loop: for(int i = 0; i < num_row_tiles; i++) {
        Col_Loop: for(int j = 0; j < num_col_tiles; j++) {
            ...
            Clear: for(int r=0; r<32; r++) {
#pragma HLS PIPELINE II=1
 VITIS_LOOP_173_1: for(int c=0; c<32; c++) tile_C[r][c] = 0;
            }
